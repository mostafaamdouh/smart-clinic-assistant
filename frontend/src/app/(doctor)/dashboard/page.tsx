...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchDoctorData = async () => {
      try {
        ...
        const safeSlots = Array.isArray(allSlots) ? allSlots : [];

        const myAppointments = safeSlots.filter(
          (slot: AppointmentSlot) =>
            slot.doctor_id === userId && slot.status === 'locked'
        );
  ...
  const handleConfirm = () =>
    toast.success('Diagnosis Confirmed! Model retraining triggered.');

  const handleCorrect = () => {
    const correctLabel = prompt(
      '❌ Enter the correct diagnosis label for the model to learn:'
    );
    if (correctLabel) {
      toast.success(
        `Diagnosis Corrected to "${correctLabel}". Retraining triggered!`
      );
    }
  };
    ...
    <div className="max-w-6xl mx-auto p-6 mt-8 space-y-8">
      ...
      <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <h2 className="text-xl font-bold text-gray-800 mb-4">
          Today's Real-time Appointments
        </h2>
                  ...
                  >
                    ...
                    <td className="p-4 flex items-center gap-3">
                      {apt.patientId ? (
                        <>
                          <Link
                            href={`/api/patient/${apt.patientId}/history`}
      ...
      <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <h2 className="text-xl font-bold text-gray-800 mb-4">
          Pending Diagnoses (CV Model)
        </h2>
        <div className="grid gap-4 md:grid-cols-2">
          {pendingDiagnoses.map((diag) => (
            <div
              key={diag.id}
              className="border p-4 rounded-xl flex gap-4 items-center bg-gray-50"
            >
              <div className="w-16 h-16 bg-white border rounded-lg flex items-center justify-center text-3xl shadow-sm">
                {diag.image}
              </div>
