...
export default function BookPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSlots((prev) =>
          prev.map((slot) =>
            slot.id === slotId
              ? { ...slot, status: 'locked', patientId: userId || undefined }
              : slot
          )
        );
      } else {
        toast.error(data?.error || 'Failed to book. Maybe someone else took it?');
      }
    } catch (error) {
  ...
  const doctorsToShow = doctors.filter((doc) =>
    suggestedDoctorIds && suggestedDoctorIds.length > 0
      ? suggestedDoctorIds.includes(doc.id)
      : true
  );
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 mt-8">
      {!selectedDoctor ? (
        <>
          <h1 className="text-3xl font-extrabold text-teal-800 tracking-tight mb-2">
            Book an Appointment
          </h1>
          <p className="text-slate-500 mb-6">
            Choose a specialist to see their available time slots.
          </p>
        ...
        <>
          ...
          <p className="text-slate-500 mb-8">
            Select a time that works for you with{' '}
          ...
          ) : availableSlots.filter((slot) => slot.doctor_id === selectedDoctor)
              .length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {availableSlots
                ...
                .map((slot) => (
                  <div
                    key={slot.id}
                    className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:border-teal-400 hover:shadow-md transition-all"
                  >
                    <div className="text-sm text-slate-500 mb-1 font-medium capitalize">
                      {slot.date}
                    </div>
